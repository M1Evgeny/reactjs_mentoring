...
class MovieListContainer extends React.Component {
    ...
    filterMovies(){
        ...
        return mockedMovies.filter((movie)=>{
            return movie.genres.includes(this.state.genre);
        })
    }
    ...
    sortMovies(movies){
        if(this.state.sortParam === 'release_date'){
            console.log(movies.sort((a,b) => new Date(b[this.state.sortParam]) - new Date(a[this.state.sortParam])));
            return movies.sort((a,b) => new Date(b[this.state.sortParam]) - new Date(a[this.state.sortParam]));
        } else {
            console.log(movies.sort((a,b) => b[this.state.sortParam] - a[this.state.sortParam]));
  ...
  render() {
      ...
      const movieFoundText = `${foundMovies.length} movie${foundMovies.length > 1 ? 's' : ''} found`;
    return (
      <React.Fragment>
        <Nav>
        ...
        <SubTitle text = {movieFoundText} />
        {
            foundMovies.length !== 0 ? <MovieList movies={foundMovies} /> : <EmptyComponent />
        }
      </React.Fragment>
    );
  }
